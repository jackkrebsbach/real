...
\begin{myproof}
  To show that $\RR$ is uncountable, suppose otherwise. Then $\RR$ can be written $\RR = \{x_1, x_2, x_3,\dots\}.$ We create a $1-1$ correspondance with the natural numnbers. Pick $x_1$ and choose an unbounded interval $I_1$ so that $x_1 \not\in I_1.$ Choose $I_2$ and $x_2$ so that $I_2 \subset I_1$ and $x_2 \not \in  I_2.$ In similar fashion create $I_3 \subset I_2$ and choose $x_3 \not\in I_3.$  We are creating a sequence of nested, close bounded intervals with $x_n \not \in I_n.$ \par
